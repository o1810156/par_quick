...
fn mid_index<T: Ord + Send>(array: &[T], a: usize, b: usize, c: usize) -> usize {
    let av = array.get(a);
    let bv = array.get(b);
    let cv = array.get(c);
...
pub fn quick_sort<T: Ord + Send>(array: &mut [T]) {
    let len = array.len();
    if len < 2 { return; }
    ...
    let (first, second) = array.split_at_mut(i);

    if i >= PARALLEL_THRESHOLD {
        rayon::join(|| quick_sort(first), || quick_sort(&mut second[1..]));
    } else {
        quick_sort(first);
        quick_sort(&mut second[1..]);
    }
}
